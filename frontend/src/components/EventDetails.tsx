    ...
    >
      <Card className="overflow-hidden">
        ...
        <div className="grid grid-cols-1 gap-px bg-border sm:grid-cols-2">
          <FriendlyRow icon={Clock} label="Quando">
            <span className="tabular-nums">{format(event._receivedAt, "HH:mm:ss")}</span>
          </FriendlyRow>
          <FriendlyRow icon={Tag} label="Voo">
            {callsign ? <span className="font-mono">{callsign}</span> : <span className="text-muted-foreground">Desconhecido</span>}
          </FriendlyRow>
          <FriendlyRow icon={Plane} label="Tipo de aeronave">
...
function WhatHappenedCard({
  ...
  const copy: Record<ReceivedEvent["action"], string> = {
    present: "Esta aeronave faz parte do snapshot atual do espaço aéreo cearense. O monitoramento a detectou como presente na área.",
    entered: "Esta aeronave acabou de aparecer na área que você está monitorando. Pode ter decolado de um aeroporto próximo ou cruzado o limite da área vinda de fora.",
    ...
    <>
      <Card className="overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-px bg-border">
          ...
          <DevRow label="on_ground" value={String(event.on_ground)} mono />
        </div>
      </Card>
...
{JSON.stringify(payload, null, 2)}
      ...
      <div className="rounded-md border border-dashed border-border bg-muted/20 p-3 text-[11px] text-muted-foreground">
        ...
        <p className="mt-1">Você está vendo campos internos e o payload JSON bruto porque o Modo Dev está ativo. Desligue-o pelo cabeçalho.</p>
      </div>
    </>
  );
}
